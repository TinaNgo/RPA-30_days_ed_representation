...
def source_of_referral_to_nominal(source: int) -> str:
    if isinstance(source, int) == False:
        return "other"
...
def preferred_language_ascl_to_nominal(language: int) -> str:
    if isinstance(language, int) == False:
        return "other"
...
df = pd.read_csv(DATA_SOURCE, names = ALL_COLUMNS, encoding = 'windows-1252')
...
df.to_csv(SPECIFIC_OUTPUT_DIR + 'manual_FS' + SUFFIX + '.csv', index=False, columns=['age', 'icu_status', 'triage_category', 'remoteness', 'repres7days'])
